// api.js — RenoVision Mobile API Layer
import Constants from 'expo-constants';

const BACKEND_URL =
  Constants.expoConfig?.extra?.kaggleBackendUrl ||
  Constants.manifest?.extra?.kaggleBackendUrl ||
  'https://ahmad3351-renovision.hf.space';

export const getBaseUrl = () => BACKEND_URL;
export const saveBaseUrl = () => {};

// Common headers — User-Agent stops Cloudflare from blocking mobile requests
const BASE_HEADERS = {
  Accept: 'application/json',
  'User-Agent': 'RenoVision-Mobile/1.0',
};
...
export const analyzeRoom = async (
  imageUri, budget, style, token, userPrompt = '', onStatus = null
) => {
  ...
  if (!health.ready) {
    ...
    if (!woke) throw new Error('Backend took too long to wake up. Please try again in 30 seconds.');
  }
    ...
    fetchWithTimeout(
      `${BACKEND_URL}/analyze`,
      {
        ...
        headers: {
          Accept: 'application/json',
          'User-Agent': 'RenoVision-Mobile/1.0',
          // NOTE: do NOT set Content-Type here — React Native sets it
  ...
  try {
    ...
    if (e.message.startsWith('non_json_response')) {
      ...
      try {
        ...
        throw new Error('Backend is still loading. Please wait 1 minute and try again.');
      }
    } else {
      throw e;
    }
  ...
  if (response.status === 401) throw new Error('401:Session expired. Please login again.');
  if (data?.error === 'outdoor_scene' || data?.is_outdoor === true) throw new Error('outdoor_scene');
  if (data?.error && response.status >= 400) throw new Error(data.error);
...
export const loginUser = async (email, password) => {
  ...
  const res = await fetchWithTimeout(
    ...
    { method: 'POST', body: fd, headers: BASE_HEADERS },
    30000
  );
  const contentType = res.headers.get('content-type') || '';
  if (!contentType.includes('application/json')) throw new Error('server_loading');
  return res.json();
};
